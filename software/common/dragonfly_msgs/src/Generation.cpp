...
<SERIALISE>
    ...
    friend std::ostream& operator<<(std::ostream& os, const <NAME>& msg)
    {
...
void generateHeaderFiles(const std::vector<MsgDesc> &msg_files, const std::filesystem::path &root_path)
{
    ...
    for(auto& msg : msg_files)
    {
        ...
        for(auto& field : msg.fields)
        {
            ...
            print += "        internal::" + std::string(field.array_size.has_value() ? "printArrayField" : "printField") + "<" + field.type + ">(os, " + "\"" + field.name + "\", msg." + field.name + ");\n";
        }
